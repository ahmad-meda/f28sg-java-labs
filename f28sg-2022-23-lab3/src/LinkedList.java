...
public class LinkedList {
	 ...
	 */
	public int size(){
		// pointing the header to the head node
		LinkedList.Node pointer_1 = headNode;
		// declaring a variable to use in for loop to store the list size
		int list_size = 0;
		while( pointer_1 != null) { // <--- run the loop until the reference of the next node is null
			
			//points the pointer to the next node every time the loop runs
			pointer_1 = pointer_1.getNextNode();
			
			// increments the size variable until the pointer encounters a null address
			list_size++;
	...
	public int total(){ 
		// pointing the header to the head node
		LinkedList.Node pointer_2 = headNode;
		//Declaring a variable to store the sum of all numbers
		int list_sum = 0;
		while(pointer_2 != null) { //<--- run the loop until the reference of the next node is null
			
			//add the current value the pointer is at to the list_sum variable 
			list_sum += pointer_2.getValue();
			
			//then move the pointer to the next node
			pointer_2 = pointer_2.getNextNode();
			
		}
		//returns the total sum of the linked list
		return list_sum; // dummy value to remove
	}
